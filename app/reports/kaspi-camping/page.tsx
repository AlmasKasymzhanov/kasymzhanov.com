...
/* ─────────────────────────── reusable bits ─────────────────────────── */
function Section({ id, num, title, sub, children }: { id: string; num: string; title: string; sub?: string; children: React.ReactNode }) { return <ResearchSection id={id} title={title} description={sub}>{children}</ResearchSection>; }

function KPI({ label, value, hint, color }: { label: string; value: string; hint: string; color: string }) { return <ResearchFact label={label} value={value} note={hint} />; }

function Callout({ icon, color, title, children }: { icon: string; color: string; title: string; children: React.ReactNode }) { return <ResearchNote title={title}>{children}</ResearchNote>; }
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "var(--font-body)" }}>
      <div style={{ maxWidth: 920, margin: "0 auto", padding: "48px 24px 80px" }}>
        ...
        {/* HEADER */}
        <div className="research-header" style={{ marginBottom: 44 }}>
          <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
            ...
            <span style={sBadge(C.amber)}>агрегированные рыночные данные · 16 мес</span>
          </div>
          <h1 style={{ fontSize: 33, fontWeight: 500, margin: "0 0 16px", letterSpacing: "-0.02em", lineHeight: 1.15 }}>
            Кемпинговые товары на Kaspi.kz — сезонная аналитика и прогноз май–сентябрь 2026
          </h1>
...
<ResearchReadingTime />
        ...
        {/* TOC */}
        <div style={{ ...sCard }}>
          <h2 style={{ fontSize: 13, fontWeight: 400, color: C.accent, margin: "0 0 12px", textTransform: "none", letterSpacing: "0.05em" }}>Содержание</h2>
          <div style={{ display: "block", marginBottom: 24 }}>
            {TOC.map(([id, t], i) => (
              <a key={id} href={`#${id}`} style={{ fontSize: 13, color: "var(--personal-text)", textDecoration: "none", padding: "6px 0", display: "flex", gap: 10 }}>
                <span style={{ color: C.accent, fontWeight: 400, minWidth: 18 }}>{i}</span>{t}
              </a>
            ))}
          </div>
        </div>
        ...
        <Section id="s0" num="0" title="Обзор">
          ...
          <Callout icon="" color={C.amber} title="Исключение, которое нужно знать">
          ...
          <Callout icon="" color={C.green} title="Качество данных подтверждено">
            Доступен полный прошлогодний сезон (май–окт 2025) — паттерн калиброван на факте, не на экстраполяции. Сверка февраля с эталоном агрегированные рыночные данные сходится в пределах ~3 %. Выводы строятся на 23 leaf-категориях с непрерывным рядом.
          </Callout>
        </Section>

        {/* 1 · DASHBOARD market curve */}
        <Section id="s1" num="1" title="Сезонность спроса" sub="Выручка ядра кемпинга помесячно, млн ₸. Столбцы — факт 2025, линия — расчётный прогноз 2026. Форма кривой повторяется каждый год.">
          <div style={sCard}>
            <ResponsiveContainer initialDimension={{width:640,height:320}} width="100%" height={340}>
              <ComposedChart accessibilityLayer aria-label="Выручка кемпинга: факт и прогноз, млн тенге" data={MARKET} margin={{ top: 10, right: 12, left: 0, bottom: 0 }}>
                <CartesianGrid stroke={C.border} strokeDasharray="3 3" vertical={false} />
                <XAxis dataKey="m" stroke={C.dim} fontSize={12} tickLine={false} />
                <YAxis stroke={C.dim} fontSize={11} tickLine={false} axisLine={false} tickFormatter={(v: number) => v.toLocaleString("ru-RU")} />
                <Tooltip content={<Tip />} cursor={{ fill: `color-mix(in srgb, ${C.accent} 6%, transparent)` }} />
                <Legend wrapperStyle={{ fontSize: 12 }} />
                <Bar fill="var(--chart-accent)" name="Факт 2025" dataKey="a" radius={[4, 4, 0, 0]} maxBarSize={42}>
                  {MARKET.map((d, i) => (
                    <Cell key={i} fill="var(--chart-accent)" />
                  ))}
                </Bar>
                <Line isAnimationActive={false} strokeDasharray="6 4" name="Прогноз 2026" dataKey="f" stroke="var(--chart-secondary)" strokeWidth={2} dot={{ r: 3, fill: "var(--chart-secondary)" }} connectNulls />
              </ComposedChart>
            </ResponsiveContainer>
        ...
        {/* 2 · pattern */}
        <Section id="s2" num="2" title="Исключительный сезонный паттерн" sub="агрегированные рыночные данные присваивает каждой категории индекс сезонности. У типового «летнего» товара маркетплейса он 0.2–0.4. В кемпинге — экстремальные значения и отношение пика к минимуму до ×18.">
          <div style={sCard}>
            <h3 style={{ fontSize: 14, fontWeight: 400, color: C.text, margin: "0 0 4px" }}>Сезонная интенсивность (пик ÷ годовой минимум)</h3>
            ...
            <ResponsiveContainer initialDimension={{width:640,height:320}} width="100%" height={300}>
              <BarChart accessibilityLayer aria-label="Сезонная интенсивность: пик к минимуму" data={INTENSITY} layout="vertical" margin={{ top: 0, right: 30, left: 30, bottom: 0 }}>
                ...
                <Tooltip content={<Tip unit="×" title="пик / минимум" />} cursor={{ fill: `color-mix(in srgb, ${C.accent} 6%, transparent)` }} />
        ...
        <Section id="s3" num="3" title="Категории в июне" sub="Размер возможности — прогноз выручки на июнь-2026, млн ₸. Палатки — крупнейший рынок сезона с большим отрывом.">
          <div style={sCard}>
            <ResponsiveContainer initialDimension={{width:640,height:320}} width="100%" height={360}>
              <BarChart accessibilityLayer aria-label="Прогноз выручки категорий на июнь 2026, млн тенге" data={TOP_JUN} layout="vertical" margin={{ top: 0, right: 40, left: 30, bottom: 0 }}>
                <CartesianGrid stroke={C.border} strokeDasharray="3 3" horizontal={false} />
                <XAxis type="number" stroke={C.dim} fontSize={11} tickLine={false} axisLine={false} tickFormatter={(v: number) => v.toLocaleString("ru-RU")} />
                <YAxis type="category" dataKey="n" stroke={C.dim} fontSize={11} width={140} tickLine={false} axisLine={false} />
                <Tooltip content={<Tip title="Июнь-2026 (прогноз)" />} cursor={{ fill: `color-mix(in srgb, ${C.accent} 6%, transparent)` }} />
                <Bar dataKey="v" radius={[0, 4, 4, 0]} maxBarSize={24}>
                  {TOP_JUN.map((d, i) => <Cell key={i} fill="var(--chart-accent)" />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
          <div style={{ display: "block", marginBottom: 24 }}>
            ...
            <Callout icon="" color={C.green} title="Портативные зарядки — +47 %">
              Растущий тренд + сезонность. агрегированные рыночные данные закладывает резкий рост YoY (91 → 134 M). Приоритетный заход на горизонте 2026–2027.
            </Callout>
            <Callout icon="" color={C.blue} title="Спальные мешки — пик в АВГУСТЕ">
        ...
        <Section id="s4" num="4" title=" Контрсезонная ловушка" sub="Эти товары формально в дереве «Туризм», но их кривая противоположна летней. Завоз под июнь = замороженные деньги. На графике: палатки (лето) против термосов и фонарей (осень–зима), факт 2025.">
          <div style={sCard}>
            <ResponsiveContainer initialDimension={{width:640,height:320}} width="100%" height={300}>
              <ComposedChart accessibilityLayer aria-label="Сезонность: сопоставление категорий, млн тенге" data={COUNTER} margin={{ top: 10, right: 12, left: 0, bottom: 0 }}>
                <CartesianGrid stroke={C.border} strokeDasharray="3 3" vertical={false} />
                <XAxis dataKey="m" stroke={C.dim} fontSize={12} tickLine={false} />
                <YAxis stroke={C.dim} fontSize={11} tickLine={false} axisLine={false} tickFormatter={(v: number) => `${v}M`} />
                <Tooltip content={<Tip />} cursor={{ fill: `color-mix(in srgb, ${C.accent} 6%, transparent)` }} />
                <Legend wrapperStyle={{ fontSize: 12 }} />
                <Bar name="Палатки (лето)" dataKey="Палатки" fill="var(--chart-accent)" radius={[4, 4, 0, 0]} maxBarSize={34} />
                <Line isAnimationActive={false} strokeDasharray="0" name="Термосы (контрсезон)" dataKey="Термосы" stroke="var(--chart-secondary)" strokeWidth={2} dot={{ r: 3 }} />
                <Line isAnimationActive={false} strokeDasharray="6 4" name="Фонари (контрсезон)" dataKey="Фонари" stroke="var(--chart-tertiary)" strokeWidth={2} dot={{ r: 3 }} />
              </ComposedChart>
            </ResponsiveContainer>
            <div style={{ fontSize: 12, color: C.dim, marginTop: 10, lineHeight: 1.6 }}>
        ...
        <Section id="s5" num="5" title="Помесячный план: Май → Сентябрь" sub="Что покупать, что продавать и чего не трогать в каждом месяце сезона.">
          {[
            ...
            <div key={x.mo} style={{ ...sCard }}>
              <div style={{ display: "flex", alignItems: "baseline", gap: 12, flexWrap: "wrap", marginBottom: 12 }}>
                <h3 style={{ fontSize: 18, fontWeight: 400, color: x.color, margin: 0 }}>{x.mo}</h3>
                <span style={{ fontSize: 13, color: C.dim }}>{x.role}</span>
              </div>
              <div style={{ display: "block", marginBottom: 24 }}>
                <div><div style={{ fontSize: 11, color: C.green, fontWeight: 400, marginBottom: 4 }}>ЗАКУП</div><div style={{ fontSize: 13, color: "var(--personal-text)", lineHeight: 1.6 }}>{x.buy}</div></div>
                <div><div style={{ fontSize: 11, color: C.blue, fontWeight: 400, marginBottom: 4 }}>ПРОДАЖА / СТОК</div><div style={{ fontSize: 13, color: "var(--personal-text)", lineHeight: 1.6 }}>{x.sell}</div></div>
                <div><div style={{ fontSize: 11, color: C.red, fontWeight: 400, marginBottom: 4 }}>НЕ ТРОГАТЬ</div><div style={{ fontSize: 13, color: "var(--personal-text)", lineHeight: 1.6 }}>{x.no}</div></div>
              </div>
            </div>
          ))}
        </Section>
        ...
        {/* 6 · full table */}
        <Section id="s6" num="6" title="Полная таблица по нишам" sub="23 категории, отсортированы по прогнозу выручки на июнь-2026. Значения — млн ₸. «Факт» — Kaspi 2025, «Прогноз» — расчётная модель 2026.">
              ...
              <thead>
                <tr>
                  {["Категория", "Тип", "пик/мин", "Пик", "25:Май", "25:Июн", "25:Июл", "25:Авг", "25:Сен", "26:Май", "26:Июн", "26:Июл", "26:Авг", "26:Сен"].map((h, i) => (
                    <th key={i} style={{ padding: "11px 10px", textAlign: i === 0 ? "left" : "center", color: C.dim, fontWeight: 500, borderBottom: `1px solid ${C.border}`, whiteSpace: "nowrap", fontSize: 11, position: i === 0 ? "sticky" : undefined, left: i === 0 ? 0 : undefined, background: i === 0 ? C.surface : undefined }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {TABLE.map((r, ri) => {
                  ...
                  return (
                    <tr key={ri} style={{ background: "transparent" }}>
                      <td style={{ padding: "9px 10px", color: C.text, fontWeight: 500, whiteSpace: "nowrap", borderBottom: `1px solid color-mix(in srgb, ${C.border} 13%, transparent)`, position: "sticky", left: 0, background: C.surface }}>{r[0]}</td>
                      <td style={{ padding: "9px 10px", textAlign: "center", color: counter ? C.amber : C.green, borderBottom: `1px solid color-mix(in srgb, ${C.border} 13%, transparent)`, whiteSpace: "nowrap", fontSize: 11 }}>{r[1]}</td>
                      <td style={{ padding: "9px 10px", textAlign: "center", color: (r[2] as number) >= 5 ? C.kaspi : "var(--personal-muted)", fontWeight: 500, borderBottom: `1px solid color-mix(in srgb, ${C.border} 13%, transparent)` }}>×{r[2]}</td>
                      <td style={{ padding: "9px 10px", textAlign: "center", color: "var(--personal-muted)", borderBottom: `1px solid color-mix(in srgb, ${C.border} 13%, transparent)`, fontSize: 11 }}>{r[3]}</td>
                      {[4, 5, 6, 7, 8].map((ci) => (
                        <HeatCell key={ci} value={Number(r[ci])} scale={{ max: Math.max(...TABLE.flatMap(row => row.slice(4).map(Number))) }} label={`${r[0]} · ${ci < 9 ? "Факт 2025" : "Прогноз 2026"} · ${["Май", "Июнь", "Июль", "Август", "Сентябрь"][(ci - 4) % 5]} · млн ₸`} />
                      ))}
          ...
          <HeatLegend label="Выручка, млн ₸ · единая шкала для всех месяцев и категорий" scale={{ max: Math.max(...TABLE.flatMap(row => row.slice(4).map(Number))) }} />
          <p style={{ fontSize: 12, color: C.dim, marginTop: 10 }}>В колонке «Тип» указаны контрсезонные позиции — внутри дерева «Туризм», но под летний закуп не подходят.</p>
        </Section>

        {/* 7 · recommendations */}
        <Section id="s7" num="7" title="Рекомендации" sub="Что делать прямо сейчас (15 мая) и стратегические приоритеты на сезон.">
          <div style={sCard}>
            <h3 style={{ fontSize: 15, fontWeight: 400, color: C.text, margin: "0 0 14px" }}>Действия сейчас (последнее окно)</h3>
            {[
              ...
              <div key={n} style={{ display: "flex", gap: 14, padding: "12px 0", borderBottom: `1px solid color-mix(in srgb, ${C.border} 19%, transparent)` }}>

                <div><span style={{ color: C.text, fontWeight: 400, fontSize: 14 }}>{t}</span> <span style={{ color: "var(--personal-muted)", fontSize: 13.5, lineHeight: 1.6 }}>{d}</span></div>
              </div>
            ))}
          </div>

          <div style={{ ...sCard }}>
            <h3 style={{ fontSize: 15, fontWeight: 400, color: C.text, margin: "0 0 14px" }}>Сводный календарь сезона</h3>
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5 }}>
                <thead><tr>{["Месяц", "Закуп", "Сток / продажа", "Не трогать"].map((h, i) => (
                  <th key={i} style={{ padding: "10px 12px", textAlign: "left", color: C.dim, fontWeight: 500, borderBottom: `1px solid ${C.border}`, fontSize: 11 }}>{h}</th>
                ))}</tr></thead>
                <tbody>
                  {[
                    ...
                    <tr key={i}>
                      <td style={{ padding: "10px 12px", color: C.text, fontWeight: 500, borderBottom: `1px solid color-mix(in srgb, ${C.border} 13%, transparent)`, whiteSpace: "nowrap" }}>{r[0]}</td>
                      <td style={{ padding: "10px 12px", color: C.green, borderBottom: `1px solid color-mix(in srgb, ${C.border} 13%, transparent)` }}>{r[1]}</td>
                      <td style={{ padding: "10px 12px", color: "var(--personal-text)", borderBottom: `1px solid color-mix(in srgb, ${C.border} 13%, transparent)` }}>{r[2]}</td>
                      <td style={{ padding: "10px 12px", color: C.red, borderBottom: `1px solid color-mix(in srgb, ${C.border} 13%, transparent)` }}>{r[3]}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <Section id="s8" num="8" title="Методология и проверка">
          <div style={sCard}>
            <p style={sP}><b style={{ color: C.text }}>Источник.</b> агрегированные рыночные данные (агрегированный массив данных, данные Kaspi.kz). История <code style={{ color: C.cyan }}>помесячный ряд</code> — помесячный факт ноя-2024 → фев-2026 (16 точек). Прогноз <code style={{ color: C.cyan }}>сезонная оценка</code> — модель «тренд × сезонность» до сен-2026, с полями индекса сезонности и пик-месяца.</p>
            ...
            <p style={sP}><b style={{ color: C.text }}>Проверка.</b> Пик-месяц сверен с фактическим максимумом помесячного ряда. Контрсезонные аномалии — термосы, фонари и печи — перепроверены вручную.</p>
            <Callout icon="" color={C.amber} title="Ограничение, учтённое в выводах">
