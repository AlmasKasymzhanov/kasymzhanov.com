  ...
  return (
    <div style={{ ...sCard, padding: "16px 0" }}>
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <div>
          <span style={{ fontSize: 11, fontWeight: 400, color, marginRight: 10, letterSpacing: "0.04em" }}>{priority}</span>
          <span style={{ fontSize: 15, fontWeight: 400, color: C.text }}>{niche}</span>
        </div>
        <div style={{ fontSize: 12, color: C.dim, fontFamily: "var(--font-mono)" }}>{size} · {growth}</div>
      </div>
      <div style={{ fontSize: 12, color: C.dim, marginTop: 8 }}>
        <strong style={{ color: C.text }}>Лидер:</strong> {leader}
      </div>
      <div style={{ fontSize: 13, color: "var(--personal-text)", marginTop: 6, lineHeight: 1.6 }}>{action}</div>
    </div>
  );
}
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "var(--font-body)" }}>
      <div style={{ maxWidth: 920, margin: "0 auto", padding: "40px 24px 80px" }}>
        ...
        <div className="research-header" style={{ marginBottom: 48, paddingBottom: 32, borderBottom: `1px solid ${C.border}` }}>
          <div style={{ display: "inline-block", padding: "16px 0", color: C.cyan, fontSize: 11, fontWeight: 400, letterSpacing: "0.05em", marginBottom: 16, textTransform: "none" }}>
...
<ResearchReadingTime />
        ...
        <Section id="sec-4" title="4. Сезонность — когда заходить">
          <div style={{ ...sCard, marginBottom: 24 }}>
            <h3 style={{ ...sH3, marginTop: 0, color: C.amber, fontSize: 18 }}> Важно: данные обрываются на февралe 2026</h3>
            <p style={sP}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
              <thead>
                <tr style={{ background: C.surface }}>
                  <th style={{ padding: "8px 10px", textAlign: "left", color: C.dim, fontWeight: 500, borderBottom: `1px solid ${C.border}`, fontSize: 10 }}>Ниша</th>
                  {["Янв", "Фев", "Мар", "Апр", "Май", "Июн", "Июл", "Авг", "Сен", "Окт", "Ноя", "Дек"].map(m => (
                    <th key={m} style={{ padding: "8px 6px", textAlign: "center", color: C.dim, fontWeight: 500, borderBottom: `1px solid ${C.border}`, fontSize: 10, minWidth: 40 }}>{m}</th>
                  ))}
                  <th style={{ padding: "8px 10px", textAlign: "left", color: C.dim, fontWeight: 500, borderBottom: `1px solid ${C.border}`, fontSize: 10 }}>Пик</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={ri}>
                    <td style={{ padding: "8px 10px", textAlign: "left", color: C.text, borderBottom: `1px solid color-mix(in srgb, ${C.border} 13%, transparent)`, fontSize: 11, fontWeight: 500 }}>
                      <span style={{ fontSize: 9, color: C.dim, marginRight: 6 }}>{row.section}</span>
                      {row.name}
                    </td>
                    {row.idx.map((v, vi) => {
                      const bg = v >= 120 ? `color-mix(in srgb, ${C.green} 19%, transparent)` : v >= 110 ? `color-mix(in srgb, ${C.green} 8%, transparent)` : v <= 80 ? `color-mix(in srgb, ${C.red} 19%, transparent)` : v <= 90 ? `color-mix(in srgb, ${C.red} 8%, transparent)` : "transparent";
                      const fg = v >= 120 ? C.green : v <= 80 ? C.red : C.text;
                      return (
                        <td key={vi} style={{ padding: "6px 6px", textAlign: "center", borderBottom: `1px solid color-mix(in srgb, ${C.border} 13%, transparent)`, background: bg, color: fg, fontSize: 11, fontWeight: v >= 120 || v <= 80 ? 500 : 400, fontVariantNumeric: "tabular-nums" }}>
                          {v}
                        </td>
                      );
                    })}
                    <td style={{ padding: "8px 10px", textAlign: "left", color: C.amber, borderBottom: `1px solid color-mix(in srgb, ${C.border} 13%, transparent)`, fontSize: 11, fontWeight: 500 }}>{row.peak}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div style={{ ...sCard, marginBottom: 12 }}>
            <h4 style={{ fontSize: 14, fontWeight: 400, color: C.green, margin: "0 0 8px" }}> Главное окно: Август-Октябрь (Kaspi Жума)</h4>
          ...
          <div style={{ ...sCard, marginBottom: 12 }}>
            <h4 style={{ fontSize: 14, fontWeight: 400, color: C.red, margin: "0 0 8px" }}> Сезонное дно: Декабрь (НГ-наборы) и Апрель (post-8 марта)</h4>
            <p style={{ ...sP, margin: 0, fontSize: 13 }}>
          ...
          <div style={{ ...sCard, marginBottom: 12 }}>
            <h4 style={{ fontSize: 14, fontWeight: 400, color: C.cyan, margin: "0 0 8px" }}> Уход за бородой — единственное исключение из правила</h4>
            <p style={{ ...sP, margin: 0, fontSize: 13 }}>
          ...
          <div style={{ ...sCard, marginBottom: 12 }}>
            <h4 style={{ fontSize: 14, fontWeight: 400, color: C.amber, margin: "0 0 8px" }}> «Составы для выпрямления» — самая сезонная ниша</h4>
            <p style={{ ...sP, margin: 0, fontSize: 13 }}>
          ...
          <div style={{ ...sCard, marginBottom: 12 }}>
            <h4 style={{ fontSize: 14, fontWeight: 400, color: C.pink, margin: "0 0 8px" }}> Аксессуары — два чётких пика (Фев + Авг)</h4>
            <p style={{ ...sP, margin: 0, fontSize: 13 }}>
        ...
        <Section id="sec-5" title="5. Детальный разбор 9 ниш">
          ...
          />
          <h4 style={{ fontSize: 14, fontWeight: 400, color: C.text, margin: "16px 0 8px" }}>Топ-10 брендов (агрегат 4 L3)</h4>
          <DataTable
        ...
        <Section id="sec-6" title="6. Барбершоп-сегмент (отдельный TAM 195M)">
          ...
          <h4 style={{ fontSize: 14, fontWeight: 400, color: C.green, margin: "20px 0 8px" }}>Фаза 1 (Месяцы 1-3): якорный вход</h4>
          <DataTable
          ...
          <h4 style={{ fontSize: 14, fontWeight: 400, color: C.accent, margin: "20px 0 8px" }}>Фаза 2 (Месяцы 4-6): расширение портфеля</h4>
          <DataTable
          ...
          <h4 style={{ fontSize: 14, fontWeight: 400, color: C.cyan, margin: "20px 0 8px" }}>Фаза 3 (Месяцы 7-12): hero-SKU и масштабирование</h4>
          <DataTable
        ...
        <Section id="sec-7" title="7. Карта конкурентов">
          ...
          <div style={{ ...sCard }}>
            <h4 style={{ fontSize: 14, fontWeight: 400, color: C.green, margin: "0 0 8px" }}>🇰🇿 Локальные KZ-лидеры</h4>
          ...
          <div style={{ ...sCard }}>
            <h4 style={{ fontSize: 14, fontWeight: 400, color: C.amber, margin: "0 0 8px" }}> Глобальные mass-market</h4>
            <p style={{ ...sP, fontSize: 13 }}>
          ...
          <div style={{ ...sCard }}>
            <h4 style={{ fontSize: 14, fontWeight: 400, color: C.pink, margin: "0 0 8px" }}> Аптечная (active cosmetics)</h4>
            <p style={{ ...sP, fontSize: 13 }}>
          ...
          <div style={{ ...sCard }}>
            <h4 style={{ fontSize: 14, fontWeight: 400, color: C.cyan, margin: "0 0 8px" }}>L&apos;Oréal Group footprint</h4>
            <p style={{ ...sP, fontSize: 13 }}>
        ...
        <Section id="sec-10" title="10. Методология">
          <h3 style={sH3}>Что входит в расчёт</h3>
          <DataTable
            headers={["Блок", "Что даёт", "Период"]}
            rows={[
              ["Рынок", "Помесячная динамика категорий, брендов, ценовых сегментов и отдельных товаров", "Полные месяцы"],
              ["Текущий срез", "Продажи, цены, активные товары и продавцы на дату анализа", "Дата указана в отчёте"],
            ]}
          />

          <h3 style={sH3}>Как мы оцениваем продажи на Kaspi</h3>
          <p style={sP}>
            Kaspi не публикует прямую выручку по каждой карточке. Поэтому показатели в отчёте являются <strong style={{ color: C.text }}>расчётной рыночной оценкой</strong>, предназначенной для сравнения категорий и товаров, а не для бухгалтерской сверки.
          </p>
          <p style={sP}>
            Все сравнения сделаны внутри одинаковых периодов и по одной логике. Чем уже уровень анализа, тем осторожнее нужно трактовать число: итоговое решение принимается после проверки кабинета продавца, поставщика и фактической экономики теста.
          </p>
          ...
          <div style={{ ...sCard }}>
            ...
            <ul style={{ ...sP, paddingLeft: 20, margin: 0 }}>
              ...
              <li>В отчёте публикуются только агрегированные показатели и выводы.</li>
          ...
          <p style={sP}>
            В каталоге Kaspi нет отдельных фильтров «термозащита» или «кератин». Поэтому функциональные ниши выделены по <strong style={{ color: C.text }}>названиям и характеристикам товаров</strong>:
          </p>
          <ul style={{ ...sP, paddingLeft: 20 }}>
        ...
        <div style={{ marginTop: 80, paddingTop: 32, borderTop: `1px solid ${C.border}`, textAlign: "center" }}>
          <p style={{ fontSize: 12, color: C.dim, margin: 0 }}>
            Отчёт подготовлен Almas Kasymzhanov · Май 2026
          </p>
          <p style={{ fontSize: 12, color: C.dim, margin: "8px 0 0" }}>
            Данные: <strong style={{ color: C.cyan }}>агрегированные рыночные данные</strong> (агрегированная аналитика Kaspi.kz) · Период: ноябрь 2024 — февраль 2026
          </p>
          <p style={{ fontSize: 12, color: C.dim, margin: "8px 0 0" }}>
