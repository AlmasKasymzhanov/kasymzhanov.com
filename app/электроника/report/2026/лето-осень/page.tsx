// @ts-nocheck
"use client";
import { HeatCell as NumericHeatCell, HeatLegend, TableSparkline, InlineDataBar } from "@/components/charts/table-visuals";
...
/* ───── design tokens ───── */
const C = {
  bg: "var(--personal-paper)", surface: "var(--personal-paper)", border: "var(--personal-border)",
...
const sH2: React.CSSProperties = { fontSize: 22, fontWeight: 400, margin: "0 0 24px", color: C.text, letterSpacing: "-0.01em", borderBottom: `1px solid ${C.border}`, paddingBottom: 12 };
const sH3: React.CSSProperties = { fontSize: 16, fontWeight: 400, margin: "28px 0 12px", color: C.text };
const sP: React.CSSProperties = { fontSize: 17, lineHeight: 1.75, color: "var(--personal-text)", margin: "0 0 12px" };
...
function Section({ id, title, defaultOpen = true, children }: { id: string; title: string; defaultOpen?: boolean; children: React.ReactNode }) { return <ResearchSection id={id} title={title}>{children}</ResearchSection>; }
...
function Sparkline({ values }: { values: (number | null)[]; color?: string; peakMonth?: number }) { return <TableSparkline values={values} labels={MONTHS} label="Сезонный индекс по месяцам, норма 100" />; }

/* Heatmap cell for month seasonal index */
function HeatCell({ value, label }: { value: number | null; label: string }) { return <NumericHeatCell value={value} label={label} scale={{ min: 0, center: 100, max: 200 }} />; }

/* Sparkbar for revenue percentage */
function Bar({ pct }: { pct: number; color?: string }) { return <InlineDataBar value={pct} label="Доля выручки среди шести показанных брендов" />; }
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "var(--font-body)" }}>
      <div style={{ maxWidth: 920, margin: "0 auto", padding: "40px 24px 80px" }}>
        ...
        <div className="research-header" style={{ marginBottom: 48, paddingBottom: 32, borderBottom: `1px solid ${C.border}` }}>
          <div style={{ display: "inline-block", padding: "16px 0", color: C.cyan, fontSize: 11, fontWeight: 400, letterSpacing: "0.05em", marginBottom: 16, textTransform: "uppercase" }}>
            Электроника · Лето-Осень 2026
          </div>
          <h1 style={{ fontSize: 32, fontWeight: 400, margin: "0 0 8px", letterSpacing: "-0.03em", lineHeight: 1.2 }}>
            Электроника, бытовая и кухонная техника на Kaspi
          </h1>
          <p className="research-lead" style={{ color: "var(--personal-muted)", fontSize: 17, margin: "12px 0 0" }}>
          ...
          <p style={{ color: C.dim, fontSize: 13, margin: "4px 0 0" }}>
            Источник: <strong style={{ color: C.cyan }}>агрегированные рыночные данные</strong> · агрегированная аналитика Kaspi.kz
          </p>
        ...
        <Section id="sec-2" title="2. Где мы сейчас (9 июня 2026)">
          <p style={sP}>
            На сегодняшнюю дату в доступном рыночном срезе доступны данные по апрель 2026 включительно (18 месяцев истории). Май и июнь 2026 — в процессе обработки.
          </p>
          ...
          <div style={{ overflowX: "auto", marginBottom: 16 }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead>
                <tr><th style={{ padding: "10px 12px", textAlign: "left", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 11 }}>Категория L1</th>
                  <th style={{ padding: "10px 12px", textAlign: "right", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 11 }}>Апрель 2026</th>
                  <th style={{ padding: "10px 12px", textAlign: "right", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 11 }}>Прогноз Июл-Сен 2026</th>
                  <th style={{ padding: "10px 12px", textAlign: "right", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 11 }}>Год к году (апрель)</th>
                </tr>
              </thead>
              <tbody>
                <tr><td style={{ padding: "10px 12px", color: C.text, borderBottom: `1px solid ${C.border}` }}> Телефоны и гаджеты</td>
                  <td style={{ padding: "10px 12px", textAlign: "right", color: "var(--personal-text)", borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>30 200 млн ₸</td>
                  <td style={{ padding: "10px 12px", textAlign: "right", color: "var(--personal-text)", borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>~102 000 млн ₸</td>
                  <td style={{ padding: "10px 12px", textAlign: "right", color: C.green, borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>+90%</td></tr>
                <tr><td style={{ padding: "10px 12px", color: C.text, borderBottom: `1px solid ${C.border}` }}> Бытовая техника</td>
                  <td style={{ padding: "10px 12px", textAlign: "right", color: "var(--personal-text)", borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>19 921 млн ₸</td>
                  <td style={{ padding: "10px 12px", textAlign: "right", color: "var(--personal-text)", borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>~62 000 млн ₸</td>
                  <td style={{ padding: "10px 12px", textAlign: "right", color: C.green, borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>+100%</td></tr>
                <tr><td style={{ padding: "10px 12px", color: C.text, borderBottom: `1px solid ${C.border}` }}> Компьютеры</td>
                  <td style={{ padding: "10px 12px", textAlign: "right", color: "var(--personal-text)", borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>8 656 млн ₸</td>
                  <td style={{ padding: "10px 12px", textAlign: "right", color: "var(--personal-text)", borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>~28 000 млн ₸</td>
                  <td style={{ padding: "10px 12px", textAlign: "right", color: C.green, borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>+75%</td></tr>
                <tr><td style={{ padding: "10px 12px", color: C.text, borderBottom: `1px solid ${C.border}` }}> ТВ, Аудио, Видео</td>
                  <td style={{ padding: "10px 12px", textAlign: "right", color: "var(--personal-text)", borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>7 397 млн ₸</td>
                  <td style={{ padding: "10px 12px", textAlign: "right", color: "var(--personal-text)", borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>~22 000 млн ₸</td>
                  <td style={{ padding: "10px 12px", textAlign: "right", color: C.green, borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>+90%</td></tr>
                <tr style={{ background: `${C.accent}` }}><td style={{ padding: "10px 12px", color: C.text, fontWeight: 400, borderBottom: `1px solid ${C.border}` }}>ИТОГО (4 L1)</td>
                  <td style={{ padding: "10px 12px", textAlign: "right", color: C.text, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>~66 174 млн ₸</td>
                  <td style={{ padding: "10px 12px", textAlign: "right", color: C.amber, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>~205 000 млн ₸</td>
                  <td style={{ padding: "10px 12px", textAlign: "right", color: C.green, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>+90%</td></tr>
              </tbody>
            </table>
          </div>

          <div style={{ ...sCard, background: `${C.amber}`, borderColor: C.amber, borderWidth: 2 }}>
        ...
        <Section id="sec-3" title="3. Сезонный паттерн рынка техники">
          <p style={sP}>
            Сезонный индекс = средняя выручка месяца / годовая средняя × 100. Значение 100 = типичный месяц. Бирюзовые ячейки показывают значения выше нормы, фиолетовые — ниже. Интенсивность растёт с отклонением от 100; значения от 200 показаны максимальным тоном, точное число сохранено.
          </p>

          <h3 style={sH3}>Тепловая карта сезонности по ключевым нишам</h3>
          <HeatLegend label="Сезонный индекс · 100 = средний месяц" scale={{ min: 0, center: 100, max: 200 }} format={v => v === 200 ? "200+" : String(v)} />
          <div style={{ overflowX: "auto", marginBottom: 16 }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
              <thead>
                <tr style={{ background: C.surface }}>
                  <th style={{ padding: "8px 10px", textAlign: "left", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 10 }}>Ниша</th>
                  {MONTHS.map(m => (
                    <th key={m} style={{ padding: "8px 6px", textAlign: "center", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 10, minWidth: 38 }}>{m}</th>
                  ))}
                  <th style={{ padding: "8px 10px", textAlign: "left", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 10 }}>Пик</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  return (
                    <tr key={n.id}>
                      <td style={{ padding: "8px 10px", textAlign: "left", color: C.text, borderBottom: `1px solid ${C.border}`, fontSize: 11, fontWeight: 500 }}>
                        {n.name}
                      </td>
                      {MONTHS.map((m, mi) => {
                        const idx = n.season_index[String(mi + 1)];
                        return <HeatCell key={mi} value={idx == null ? null : Number(idx)} label={`${n.name} · ${m} · сезонный индекс, норма 100`} />;
                      })}
                      <td style={{ padding: "8px 10px", color: C.amber, borderBottom: `1px solid ${C.border}`, fontSize: 11, fontWeight: 400 }}>{MONTHS[Number(peakM[0]) - 1]} ({String(peakM[1])})</td>
                    </tr>
                  );
                })}
              </tbody>
          ...
          <div style={{ ...sCard, marginBottom: 12 }}>
            <h4 style={{ fontSize: 14, fontWeight: 400, color: C.green, margin: "0 0 8px" }}> Главное окно: август-октябрь (Kaspi Жума + back-to-school)</h4>
          ...
          <div style={{ ...sCard, marginBottom: 12 }}>
            <h4 style={{ fontSize: 14, fontWeight: 400, color: C.amber, margin: "0 0 8px" }}> Самая сильная сезонность: отопительные котлы</h4>
            <p style={{ ...sP, margin: 0, fontSize: 13 }}>
          ...
          <div style={{ ...sCard, marginBottom: 12 }}>
            <h4 style={{ fontSize: 14, fontWeight: 400, color: C.cyan, margin: "0 0 8px" }}> Чистый летний пик: кондиционеры, вентиляторы</h4>
            <p style={{ ...sP, margin: 0, fontSize: 13 }}>
          ...
          <div style={{ ...sCard, marginBottom: 12 }}>
            <h4 style={{ fontSize: 14, fontWeight: 400, color: C.pink, margin: "0 0 8px" }}> Back-to-school (август-сентябрь)</h4>
            <p style={{ ...sP, margin: 0, fontSize: 13 }}>
        ...
        <Section id="sec-4" title="4. Прогноз: июль-сентябрь 2026">
          ...
          <h3 style={sH3}>Топ-15 ниш по прогнозируемой выручке июль-сентябрь 2026</h3><p style={{ ...sP, fontSize: 12, color: C.dim }}>Мини-график показывает форму сезонности за январь–декабрь: нулевая база, индивидуальный максимум строки. Для сравнения уровня индексов используйте тепловую карту выше. Точные значения доступны при фокусе или нажатии.</p>
          <div style={{ overflowX: "auto", marginBottom: 16 }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead>
                <tr>
                  <th style={{ padding: "10px 8px", textAlign: "left", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 11 }}>#</th>
                  <th style={{ padding: "10px 8px", textAlign: "left", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 11 }}>Ниша</th>
                  <th style={{ padding: "10px 8px", textAlign: "right", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 11 }}>Апрель 26</th>
                  <th style={{ padding: "10px 8px", textAlign: "right", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 11 }}>Прогноз Jul-Sep</th>
                  <th style={{ padding: "10px 8px", textAlign: "right", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 11 }}>Год к году</th>
                  <th style={{ padding: "10px 8px", textAlign: "left", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 11 }}>Сезонность, янв.–дек.</th>
                </tr>
              </thead>
              <tbody>
                {DATA.top30.slice(0, 15).map((n: any, i: number) => {
                  ...
                  const sparkVals = Array.from({ length: 12 }, (_, m) => n.season_index[String(m+1)] == null ? null : Number(n.season_index[String(m+1)]));
                  return (
                    <tr key={n.id}>
                      <td style={{ padding: "10px 8px", textAlign: "left", color: C.text, borderBottom: `1px solid ${C.border}` }}>{i+1}</td>
                      <td style={{ padding: "10px 8px", textAlign: "left", color: C.text, borderBottom: `1px solid ${C.border}`, fontSize: 12, fontWeight: 500 }}>
                        {n.name}
                        <div style={{ fontSize: 10, color: C.dim, marginTop: 2 }}>{n.l1_name} → {n.l2_name}</div>
                      </td>
                      <td style={{ padding: "10px 8px", textAlign: "right", color: "var(--personal-text)", borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>{fmtM(n.revenue_apr26)}</td>
                      <td style={{ padding: "10px 8px", textAlign: "right", color: C.amber, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>{fmtM(n.forecast)}</td>
                      <td style={{ padding: "10px 8px", textAlign: "right", color: n.yoy_apr > 1.5 ? C.green : "#ccc", borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>{(n.yoy_apr * 100).toFixed(0)}%</td>
                      <td style={{ padding: "10px 8px", textAlign: "left", borderBottom: `1px solid ${C.border}` }}><Sparkline values={sparkVals} peakMonth={peakMonth} /></td>
                    </tr>
                  );
                })}
              </tbody>
          ...
          <div style={{ ...sCard, background: `${C.green}`, borderColor: C.green, marginTop: 24 }}>
        ...
        <Section id="sec-5" title="5. Топ-30 ниш по прогнозу июль-сентябрь 2026" defaultOpen={false}>
          <p style={sP}>Полный рейтинг 30 ниш техники. Годовая динамика:  &gt;+150% ·  +100-150% ·  &lt;+100%. Индекс лета–осени:  ≥120 (сильная сезонность) ·  80-119 ·  ≤79 (слабая).</p>
          <div style={{ overflowX: "auto", marginBottom: 16 }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11.5 }}>
              <thead>
                <tr><th style={{ padding: "8px 6px", textAlign: "left", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 10 }}>#</th>
                  <th style={{ padding: "8px 6px", textAlign: "left", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 10 }}>Ниша</th>
                  <th style={{ padding: "8px 6px", textAlign: "right", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 10 }}>Прогноз</th>
                  <th style={{ padding: "8px 6px", textAlign: "right", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 10 }}>YoY</th>
                  <th style={{ padding: "8px 6px", textAlign: "right", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 10 }}>Сезон</th>
                  <th style={{ padding: "8px 6px", textAlign: "right", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 10 }}>Брендов</th>
                  <th style={{ padding: "8px 6px", textAlign: "right", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 10 }}>SKU</th>
                </tr>
              </thead>
              <tbody>
                {DATA.top30.map((n: any, i: number) => (
                  <tr key={n.id}>
                    <td style={{ padding: "7px 6px", color: C.dim, borderBottom: `1px solid ${C.border}` }}>{i+1}</td>
                    <td style={{ padding: "7px 6px", color: C.text, borderBottom: `1px solid ${C.border}` }}>
                      {n.name}
                      <div style={{ fontSize: 10, color: C.dim, marginTop: 2 }}>{n.l1_name}</div>
                    </td>
                    <td style={{ padding: "7px 6px", textAlign: "right", color: C.amber, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>{fmtM(n.forecast)}</td>
                    <td style={{ padding: "7px 6px", textAlign: "right", color: n.yoy_apr > 2.5 ? C.green : n.yoy_apr > 2 ? C.cyan : "#ccc", borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>{(n.yoy_apr * 100).toFixed(0)}%</td>
                    <td style={{ padding: "7px 6px", textAlign: "right", color: n.summer_autumn_index >= 120 ? C.green : n.summer_autumn_index <= 80 ? C.red : "#ccc", fontWeight: n.summer_autumn_index >= 120 ? 500 : 400, borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>{n.summer_autumn_index}</td>
                    <td style={{ padding: "7px 6px", textAlign: "right", color: "var(--personal-text)", borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>{n.brands_apr26}</td>
                    <td style={{ padding: "7px 6px", textAlign: "right", color: "var(--personal-text)", borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>{fmtNum(n.skus_apr26)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div style={{ overflowX: "auto", marginBottom: 16 }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11.5 }}>
              <thead>
                <tr><th style={{ padding: "8px 6px", textAlign: "left", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 10 }}>Ниша</th>
                  <th style={{ padding: "8px 6px", textAlign: "right", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 10 }}>Апрель</th>
                  <th style={{ padding: "8px 6px", textAlign: "right", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 10 }}>Прогноз</th>
                  <th style={{ padding: "8px 6px", textAlign: "right", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 10 }}>Лет-осень</th>
                  <th style={{ padding: "8px 6px", textAlign: "right", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 10 }}>Брендов</th>
                  <th style={{ padding: "8px 6px", textAlign: "left", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 10 }}>Логика</th>
                </tr>
              </thead>
              <tbody>
                {DATA.extras.map((n: any) => (
                  <tr key={n.id}>
                    <td style={{ padding: "7px 6px", color: C.text, borderBottom: `1px solid ${C.border}` }}>{n.name}</td>
                    <td style={{ padding: "7px 6px", textAlign: "right", color: "var(--personal-text)", borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>{fmtM(n.revenue_apr26)}</td>
                    <td style={{ padding: "7px 6px", textAlign: "right", color: C.amber, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontVariantNumeric: "tabular-nums" }}>{fmtM(n.forecast)}</td>
                    <td style={{ padding: "7px 6px", textAlign: "right", color: C.green, fontWeight: 400, borderBottom: `1px solid ${C.border}` }}>{n.summer_autumn_index}</td>
                    <td style={{ padding: "7px 6px", textAlign: "right", color: "var(--personal-text)", borderBottom: `1px solid ${C.border}` }}>{n.brands_apr26}</td>
                    <td style={{ padding: "7px 6px", color: C.dim, borderBottom: `1px solid ${C.border}`, fontSize: 11 }}>
        ...
        <Section id="sec-6" title="6. Разбор 7 приоритетных ниш + отзывы покупателей">
          ...
          {DATA.priority.map((n: any, ni: number) => {
            ...
            return (
              <div key={n.id} style={{ marginTop: 36, paddingTop: 24 }}>
                ...
                {/* Top brands */}
                <h4 style={{ fontSize: 13, fontWeight: 400, color: C.text, margin: "16px 0 8px", textTransform: "uppercase", letterSpacing: "0.04em" }}>Топ-6 брендов</h4><p style={{fontSize: 12, color: C.dim}}>Длина полосы — доля выручки среди шести показанных брендов, 0–100%.</p>
                      ...
                      {n.top_brands.slice(0, 6).map((b, bi) => {
                        ...
                        return (
                          <tr key={b.brand}>
                            ...
                            <td style={{ padding: "6px 8px", textAlign: "right", color: "var(--personal-text)", fontVariantNumeric: "tabular-nums" }}>{fmtM(b.revenue)}</td>
                ...
                {/* Top SKUs */}
                <h4 style={{ fontSize: 13, fontWeight: 400, color: C.text, margin: "16px 0 8px", textTransform: "uppercase", letterSpacing: "0.04em" }}>Топ-5 SKU</h4>
                <div style={{ overflowX: "auto", marginBottom: 12 }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11.5 }}>
                    <tbody>
                      {n.top_skus.slice(0, 5).map((s, si) => (
                        <tr key={s.sku}>
                          ...
                          <td style={{ padding: "6px 8px", color: C.text, fontSize: 11 }}>
                            <a href={s.sku_url} target="_blank" rel="noopener" style={{ color: C.text, textDecoration: "none" }}>
                          ...
                          <td style={{ padding: "6px 8px", textAlign: "right", color: "var(--personal-text)", fontVariantNumeric: "tabular-nums", whiteSpace: "nowrap" }}>{fmtNum(s.price)} ₸</td>
                          <td style={{ padding: "6px 8px", textAlign: "right", color: C.amber, fontWeight: 400, fontVariantNumeric: "tabular-nums" }}>{fmtM(s.revenue)}</td>
                ...
                {n.segments.length > 0 && (
                  <>
                    <h4 style={{ fontSize: 13, fontWeight: 400, color: C.text, margin: "16px 0 8px", textTransform: "uppercase", letterSpacing: "0.04em" }}>Ценовые сегменты</h4>
                    <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
                      {n.segments.map(s => (
                        <div key={s.segment} style={{ flex: 1, padding: "16px 0" }}>
                          <div style={{ fontSize: 10, color: C.dim, textTransform: "uppercase", letterSpacing: "0.06em" }}>{s.segment_name}</div>
                          <div style={{ fontSize: 13, color: C.text, fontWeight: 400, marginTop: 4, fontVariantNumeric: "tabular-nums" }}>{fmtNum(Math.round(s.median_price))} ₸</div>
                          <div style={{ fontSize: 11, color: s.revenue_share_pct > 30 ? color : C.dim, marginTop: 2, fontWeight: 400 }}>{s.revenue_share_pct.toFixed(0)}% выручки</div>
                        </div>
                      ))}
                    </div>
                  </>
                ...
                {(n.top_complaints.length > 0 || n.sample_complaints.length > 0) && (
                  <>
                    <h4 style={{ fontSize: 13, fontWeight: 400, color: C.text, margin: "16px 0 8px", textTransform: "uppercase", letterSpacing: "0.04em" }}>
                      Что улучшать (по разбору {n.total_reviews_analyzed} отзывов)
                    </h4>
                    {n.top_complaints.length > 0 && (
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
                        {n.top_complaints.map(c => (
                          <div key={c.theme} style={{ padding: "16px 0", fontSize: 12, color: C.text }}>
                            <span style={{ color: C.red, fontWeight: 400, marginRight: 4 }}>{c.count}×</span>{c.theme}
                          </div>
                        ))}
                      </div>
                    )}
                    {n.sample_complaints.slice(0, 3).map((s, si) => (
                      <div key={si} style={{ padding: "16px 0", marginBottom: 8, fontSize: 12 }}>
                        <div style={{ color: C.dim, fontSize: 10, marginBottom: 4 }}>
                          <strong style={{ color: C.red }}>★ {s.rating}</strong> · {s.theme} · {(s.sku_name || "").slice(0, 60)}
                        </div>
                        <div style={{ color: "var(--personal-text)", fontStyle: "italic", lineHeight: 1.6 }}>«{s.excerpt}»</div>
                      </div>
                    ))}
                  </>
                )}
                ...
                {n.top_positives.length > 0 && (
                  <>
                    <h4 style={{ fontSize: 13, fontWeight: 400, color: C.text, margin: "16px 0 8px", textTransform: "uppercase", letterSpacing: "0.04em" }}>Что нравится в лидерах</h4>
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
                      {n.top_positives.map(p => (
                        <div key={p.theme} style={{ padding: "16px 0", fontSize: 12, color: C.text }}>
                          <span style={{ color: C.green, fontWeight: 400, marginRight: 4 }}>{p.count}×</span>{p.theme}
                        </div>
                      ))}
                    </div>
                  </>
        ...
        <Section id="sec-7" title="7. Рекомендации по запускам">
          ...
          <div style={{ overflowX: "auto", marginBottom: 16 }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11.5 }}>
              <thead>
                <tr><th style={{ padding: "8px 8px", textAlign: "left", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 10 }}>Приоритет</th>
                  <th style={{ padding: "8px 8px", textAlign: "left", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 10 }}>Ниша</th>
                  <th style={{ padding: "8px 8px", textAlign: "left", color: C.dim, fontWeight: 400, borderBottom: `1px solid ${C.border}`, fontSize: 10 }}>УТП на основе разбора отзывов</th>
                </tr>
              </thead>
              <tbody>
                <tr><td style={{ padding: "10px 8px", color: C.green, fontWeight: 400, borderBottom: `1px solid ${C.border}` }}> №1</td>
                  <td style={{ padding: "10px 8px", color: C.text, fontWeight: 400, borderBottom: `1px solid ${C.border}` }}>Отопительные котлы<div style={{ fontSize: 10, color: C.dim }}>Прогноз ~3.5 млрд за квартал</div></td>
                  <td style={{ padding: "10px 8px", color: "var(--personal-text)", borderBottom: `1px solid ${C.border}`, fontSize: 12 }}>Сильнейшая сезонность (индекс сентября 228). Из отзывов: <strong style={{ color: C.text }}>проблем у топ-SKU нет</strong> (Navien держит ★4.96). Это «защищённая» категория — конкурировать нужно сервисом и установкой, а не продуктом. Партнёрство с монтажниками в Алматы/Астане.</td></tr>
                <tr><td style={{ padding: "10px 8px", color: C.green, fontWeight: 400, borderBottom: `1px solid ${C.border}` }}> №2</td>
                  <td style={{ padding: "10px 8px", color: C.text, fontWeight: 400, borderBottom: `1px solid ${C.border}` }}>Очистители и увлажнители<div style={{ fontSize: 10, color: C.dim }}>+167% год к году, 1.4 млрд прогноз</div></td>
                  <td style={{ padding: "10px 8px", color: "var(--personal-text)", borderBottom: `1px solid ${C.border}`, fontSize: 12 }}>Жалобы: <strong style={{ color: C.text }}>«не оправдал ожиданий», «непонятно как очищает», «шумит ночью»</strong>. УТП — <strong style={{ color: C.green }}>прозрачный индикатор PM2.5 + ночной режим &lt;30 дБ + понятный гайд</strong>. Целевой сегмент — Средний (P3, 15-25 тыс ₸). Конкурент Xiaomi доминирует функциями, но не объясняет ценность.</td></tr>
                <tr><td style={{ padding: "10px 8px", color: C.cyan, fontWeight: 400, borderBottom: `1px solid ${C.border}` }}> №3</td>
                  <td style={{ padding: "10px 8px", color: C.text, fontWeight: 400, borderBottom: `1px solid ${C.border}` }}>Морозильники<div style={{ fontSize: 10, color: C.dim }}>5.2 млрд прогноз, заготовки</div></td>
                  <td style={{ padding: "10px 8px", color: "var(--personal-text)", borderBottom: `1px solid ${C.border}`, fontSize: 12 }}>Жалобы на Бирюсу (★4.37): <strong style={{ color: C.text }}>«шумит», «бракованный», «доставка повредила»</strong>. УТП — <strong style={{ color: C.green }}>усиленная упаковка для доставки + бесплатная замена при повреждении + работа &lt;42 дБ</strong>. Объём 100-200 л для квартир, цена P3 (60-90 тыс ₸).</td></tr>
                <tr><td style={{ padding: "10px 8px", color: C.cyan, fontWeight: 400, borderBottom: `1px solid ${C.border}` }}> №4</td>
                  <td style={{ padding: "10px 8px", color: C.text, fontWeight: 400, borderBottom: `1px solid ${C.border}` }}>Электрочайники и термопоты<div style={{ fontSize: 10, color: C.dim }}>0.9 млрд прогноз</div></td>
                  <td style={{ padding: "10px 8px", color: "var(--personal-text)", borderBottom: `1px solid ${C.border}`, fontSize: 12 }}>Жалобы: <strong style={{ color: C.text }}>«запах пластика» (4×), «шум» (4×), «сломался через 2 месяца»</strong>. УТП — <strong style={{ color: C.green }}>стекло + сталь без пластика внутри + двойные стенки (тихий + не обжигает) + 2 года гарантии</strong>. Конкурент Tefal недорогой, но «пластиковый запах». Smeg премиум — но шумный. Окно — стекло-сталь в среднем сегменте (12-18 тыс ₸).</td></tr>
                <tr><td style={{ padding: "10px 8px", color: C.cyan, fontWeight: 400, borderBottom: `1px solid ${C.border}` }}> №5</td>
                  <td style={{ padding: "10px 8px", color: C.text, fontWeight: 400, borderBottom: `1px solid ${C.border}` }}>Отпариватели<div style={{ fontSize: 10, color: C.dim }}>1.1 млрд прогноз, индекс 138</div></td>
                  <td style={{ padding: "10px 8px", color: "var(--personal-text)", borderBottom: `1px solid ${C.border}`, fontSize: 12 }}>Жалобы на Tefal IXEO+: <strong style={{ color: C.text }}>«трубки вылетают», «недогладилка шатается», «шумит сильно»</strong>. На Jiffy Steamer: «маленькая площадь поверхности». УТП — <strong style={{ color: C.green }}>широкая подошва 12 см + усиленные крепления трубок + тихий клапан</strong>. P3-P4 сегмент (18-35 тыс ₸).</td></tr>
                <tr><td style={{ padding: "10px 8px", color: C.cyan, fontWeight: 400, borderBottom: `1px solid ${C.border}` }}> №6</td>
                  <td style={{ padding: "10px 8px", color: C.text, fontWeight: 400, borderBottom: `1px solid ${C.border}` }}>Утюги<div style={{ fontSize: 10, color: C.dim }}>0.8 млрд прогноз, back-to-school</div></td>
                  <td style={{ padding: "10px 8px", color: "var(--personal-text)", borderBottom: `1px solid ${C.border}`, fontSize: 12 }}>Главная жалоба: <strong style={{ color: C.red }}>«не работает с первого включения» (7×)</strong>. На Vitek: «вода течёт». На Tefal Virtuo: «маленький, как игрушечный». УТП — <strong style={{ color: C.green }}>заводская проверка каждого утюга + bigger подошва 12-14 см + анти-капля система с гарантией возврата</strong>. P2-P3 сегмент (8-15 тыс ₸).</td></tr>
                <tr><td style={{ padding: "10px 8px", color: C.amber, fontWeight: 400, borderBottom: `1px solid ${C.border}` }}> №7</td>
                  <td style={{ padding: "10px 8px", color: C.text, fontWeight: 400, borderBottom: `1px solid ${C.border}` }}>Фитнес-браслеты<div style={{ fontSize: 10, color: C.dim }}>+232% год к году, 1.1 млрд</div></td>
                  <td style={{ padding: "10px 8px", color: "var(--personal-text)", borderBottom: `1px solid ${C.border}`, fontSize: 12 }}>Жалобы на WHOOP: <strong style={{ color: C.text }}>«приложение только на английском», «нет инструкции», «застёжка не держится»</strong>. УТП — <strong style={{ color: C.green }}>приложение на русском + казахском + видео-инструкция + усиленный замок</strong>. Сегмент P3 (20-35 тыс ₸), позиция «WHOOP на русском».</td></tr>
              </tbody>
            </table>
          </div>

          <div style={{ ...sCard, background: `${C.amber}`, borderColor: C.amber, marginTop: 24 }}>
        ...
        <Section id="sec-8" title="8. Методология">
          ...
          <ul style={{ ...sP, paddingLeft: 20 }}>
            ...
            <li>Никаких чужих кабинетов, сессий и персональных данных</li>
        ...
        {/* Footer */}
        <div style={{ marginTop: 80, paddingTop: 32, textAlign: "center" }}>
          <p style={{ fontSize: 12, color: C.dim, margin: 0 }}>
            Отчёт подготовлен Almas Kasymzhanov · 9 июня 2026
          </p>
          <p style={{ fontSize: 12, color: C.dim, margin: "8px 0 0" }}>
            Данные: <strong style={{ color: C.cyan }}>агрегированные рыночные данные</strong> (агрегированная аналитика Kaspi.kz) · 18 месяцев истории
          </p>
          <p style={{ fontSize: 12, color: C.dim, margin: "8px 0 0" }}>
